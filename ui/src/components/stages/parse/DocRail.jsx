...
/** Vertical filmstrip of docs — icon + short code + status badge. */
export function DocRail({ docs, activeId, onActive }) {
  return (
    <aside className="bg-white border-r border-line overflow-auto flex-shrink-0" style={{ width: 72 }}>
      <div className="p-2 space-y-1.5">
        {docs.map(d => {
          ...
          return (
            ...
            >
              <div
                className="w-11 h-11 rounded-[6px] border-2 flex items-center justify-center text-[16px] relative"
                style={{ borderColor: ringColor, color: t.color, background: t.color + '10' }}
              >
                {t.icon}
                {failed && (
                  <span className="absolute -top-1.5 -right-1.5 w-[16px] h-[16px] rounded-full bg-status-red text-white text-[9px] flex items-center justify-center">
                    ✕
                  </span>
                )}
                {!failed && reviewed && (
                  <span className="absolute -top-1.5 -right-1.5 w-[16px] h-[16px] rounded-full bg-status-green text-white text-[9px] flex items-center justify-center">
                    ✓
                  </span>
                )}
                {!failed && !reviewed && extracted && (
                  <span className="absolute -top-1.5 -right-1.5 w-[16px] h-[16px] rounded-full bg-status-gold text-white text-[8px] font-bold flex items-center justify-center font-mono">
                    !
                  </span>
                )}
              </div>
