 ...
 *
 * <p>So blocks declare themselves {@link Stability#STABLE} or {@link Stability#VOLATILE} and
 * this renders stable first, whatever order they were added in. It is the text-side mirror of
 * what the vision path already does by putting images ahead of the instruction, and for the
 * same reason.
 *
 * <p>The second use is the derivation cache. {@link #volatileDigest()} hashes only the part
 * that varies, so an entry is keyed on what actually distinguishes one call from another —
 * and re-wording a vocabulary block does not invalidate every stored answer. Where a prompt
 * edit <em>should</em> invalidate them, key on {@link #digest()} instead.
 *
...
public final class PromptContext {
    ...
    /** Whether a block is the same on every call, or the reason this call is different. */
    ...
    /** A section that is why this call is not the last one: the case's own material. */
    ...
    /** The whole prompt, stable sections first, in the order each was added. */
    public String render() {
        return render(Stability.STABLE) + render(Stability.VOLATILE);
    }
    ...
    /** Only what varies. Use as a cache key where the invariant half is not worth re-keying. */
    public String volatileDigest() {
        return sha256Hex(render(Stability.VOLATILE));
    }
