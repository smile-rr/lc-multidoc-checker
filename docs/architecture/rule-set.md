...
|---|---|---|---|---|---|---|
| `DATE-01` | Doc dates within valid window | any presented doc | — | — | PROG_AGENT | MAJOR |
...
| `AMT-01` | Invoice amount/currency conform to LC, within tolerance | `INV` | — | — | PROG_AGENT | CRITICAL |
...
| `TRANS-01` | B/L bears on-board notation and date | `BOL` | — | — | PROG_AGENT | CRITICAL |
| `TRANS-02` | B/L is clean (no defect/damage clauses) | `BOL` | — | — | AGENT | MAJOR |
| `TRANS-03` | Ports of loading and discharge match LC | `BOL` | — | `:44E:` and/or `:44F:` populated | PROG_AGENT | CRITICAL |
| `TRANS-04` | Freight terms conform to LC and Incoterm | `BOL`, `INV` | — | — | PROG_AGENT | MAJOR |
...
| `PARTY-02` | BOE drawee matches LC stipulated drawee | `BOE` | — | `:42A:` or `:42C:` or `:42D:` populated | PROG_AGENT | CRITICAL |
| `XD-01` | Quantity, weight, marks consistent across INV/PKL/BOL | at least 2 of `[INV, PKL, BOL]` | — | — | PROG_AGENT | MAJOR |
...
| `CERT-02` | Certificate signature and form requirements | `BC` OR `WC` | — | LC stipulates signature requirements | PROG_AGENT | MAJOR |
| `COND-DYN` | :47A: free-text condition generator (architecture, not a catalog row) | — | — | `:47A:` populated | AGENT_TOOL | varies |
...
compliance concern, programmatic and agent cooperate inside a rule
where useful, cross-doc concerns consolidated.
...
**Type:** PROG_AGENT · **Severity:** MAJOR
...
PROG part computes the per-doc-type window; the AGENT part disambiguates
"issued" vs "executed" dates that BC/WC sometimes carry separately.
...
**Type:** PROG_AGENT · **Severity:** CRITICAL
...
date unless an explicit on-board notation states otherwise. The rule
...
those ports. The PROG part performs literal match; the AGENT part
handles generic LC stipulations ("ANY CHINESE PORT", "EUROPEAN PORT")
and `:47A:` overrides.
...
the LC permits it; that prohibition is also encoded here.
...
named in `:42A:`/`:42C:`/`:42D:` of the LC. The PROG part matches BIC /
identity strings; the AGENT part reconciles addresses and minor name
variants.
...
shipping marks must agree across documents. The PROG part performs
numeric reconciliation with unit normalization (kg ↔ MT, etc.) and
checks shipping-mark string presence. The AGENT part handles the
structural cases: per-carton breakdown vs aggregate; consolidated-
container manifests; packing-list totals matching invoice line-item
sums; multi-line marks blocks.
...
authored by the stipulated party. PROG part: signature_present flag.
AGENT part: signed-by reconciliation when LC names a party
("signed by beneficiary", "signed by inspection agent").
...
## 3. The :47A: dynamic rule generator (architecture, not catalog)
...
The generator runs ONE LLM call per session (plus tool rounds if
AGENT_TOOL), bounded. The catalog has no static `COND-*` rule.
...
## 8. Decisions needed before Phase-2 spike plan

1. **Rule total: 18 static + 1 dynamic generator.** Acceptable, or
   prune/expand?
2. **Severity assignments above** — any specific rule you'd reclassify?
