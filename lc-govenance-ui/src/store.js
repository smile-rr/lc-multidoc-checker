...
export function deriveVals(state, setState) {
  ...
  function buildCheck(c, ctx) {
    ...
    return {
      ...
      onDelete: () => requestConfirm({ title: 'Delete check?', message: `“${title}” (${c.id}) will be permanently removed.`, confirmLabel: 'Delete check', onConfirm: () => deleteCheck(c.id) }),
  ...
  return {
    ...
    newCheck: () => setState((s) => { const id = 'CHK-NEW' + (s.newSeq + 1); const nc = { id, domain: 'Uncategorised', cases: 0, draft: true, agentId: null, groupId: null, title: 'New check', severity: 'MAJOR', refs: [], suggestion: 'Add a condition or two so the assistant has something to check.', body: 'Describe what the agent should check, in plain language.\n\nFields to look at: {  }\n\n- ', timeline: [{ color: 'var(--me-blue)', label: 'Created', date: 'just now', detail: 'New check.' }] }; return { extraChecks: [nc, ...s.extraChecks], newSeq: s.newSeq + 1, editingId: id, section: 'checks' } }),
    ...
    confirm: S.confirm ? { title: S.confirm.title, message: S.confirm.message, confirmLabel: S.confirm.confirmLabel, onCancel: () => setState({ confirm: null }), onConfirm: () => { const fn = S.confirm.onConfirm; setState({ confirm: null }); if (fn) fn() } } : null,
