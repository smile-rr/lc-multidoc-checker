    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <input
          ...
          style={{ flex: 1, minWidth: 0, fontSize: 15.5, fontWeight: 700, color: 'var(--me-ink)', padding: '4px 6px', marginLeft: -6 }}
        />
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--me-grey-70)' }}>{check.id}</span>
        {check.draft && (
          ...
          {menuOpen && (
            <div style={{ ...popover, top: 32, right: 0, width: 176 }}>
              <button onClick={() => { setMenuOpen(false); check.onToggleInactive() }} style={menuItem}>
                ...
                {check.inactive ? 'Activate' : 'Deactivate'}
              </button>
              <button onClick={() => { setMenuOpen(false); check.onDelete() }} style={{ ...menuItem, color: 'var(--status-error)' }}>
